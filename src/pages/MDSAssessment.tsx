...
export const MDSAssessment: React.FC = () => {
  ...
  const handlePrint = async () => {
    console.log('MDS Assessment: Starting PDF generation...');
    try {
      setIsGeneratingPDF(true);
      const formData = getValues();
      console.log('MDS Assessment: Form data for PDF:', formData);
      const success = await generateFormPDF(FORM_NAME, formData);
      
      if (!success && formRef.current) {
        console.log('MDS Assessment: Template PDF failed or not found, falling back to exportToPDF...');
        const { exportToPDF } = await import('../utils/pdfExport');
        await exportToPDF(formRef.current, `MDS_Assessment_${new Date().toISOString().split('T')[0]}.pdf`);
      }
      console.log('MDS Assessment: PDF generation successful.');
    } catch (error) {
      console.error('MDS Assessment: PDF error:', error);
